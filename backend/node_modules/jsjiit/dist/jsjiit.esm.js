...
var Semester = class _Semester {
  ...
  static from_json(resp, options = {}) {
    return new _Semester(resp.registrationcode, resp.registrationid, options);
  }
};
...
${JSON.stringify(resp.status, null, 2)}`);
  ...
  async get_semesters_for_exam_events() {
    ...
    return resp["response"]["semesterCodeinfo"]["semestercode"].map(
      (i) => Semester.from_json(i)
    );
  }
  /**
  ...
  async get_exam_events(semester) {
    ...
    return resp["response"]["eventcode"]["examevent"].map(
      (i) => ExamEvent.from_json(i)
    );
  }
  /**
  ...
  async download_grades(semester) {
    const ENDPOINT = "/studentsgpacgpa/semesterwisestudentresultreport/" + this.session.instituteid + "/" + semester.registration_id + "/" + semester.registration_code;
    const localname = await generate_local_name();
    let _headers = await this.session.get_headers(localname);
    const fetchOptions = {
      method: "GET",
  ...
  async get_semesters_for_grade_card() {
    ...
    const completeSemesters = (gradeCardResp["response"]?.["registrations"] || []).map(
      (i) => Semester.from_json(i, {
    ...
    const choicePrintSemesters = (choicePrintResp["response"]?.["registrationcodelist"] || []).map((i) => {
      const isComplete = completeRegistrationIds.has(i.registrationid) || completeRegistrationCodes.has(i.registrationcode);
      return Semester.from_json(i, {
        is_grade_card_complete: isComplete,
        grade_card_source: isComplete ? "studentgradecard" : "studentchoiceprint"
      });
    });
    const semestersByRegistrationId = /* @__PURE__ */ new Map();
  ...
  async fill_feedback_form(feedback_option) {
    ...
    for (let question_feedback_payload of question_feedback_payload_array) {
      try {
        const questions_api_resp2 = await this.__hit(
          "POST",
